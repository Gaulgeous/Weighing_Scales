	...
	if (!wifiSentStat[CWM]) {
		...
		// Wait for the OK signal to return
			...
			if (currentTime - previousTime > CHECK_TIME) {
				return_state(*huart1, uart2Counter, screenData, htim2);
 ...
 */
int gui_flag_helper(int input, ScreenData* data, int* pos, TIM_HandleTypeDef* htim2) {
...
int parse_helper1(int input, ScreenData* data, int* pos, TIM_HandleTypeDef* htim2) {
	...
	switch(*pos) {
		...
		case RAW:
			retValue = gui_flag_helper(input, data, pos, htim2);
			break;
