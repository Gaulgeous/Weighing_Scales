...
int main(void)
{
  ...
  while (1)
  {
	  if (!wifiStat[SUB]) {
		if (!wifiStat[RST]) {
			 at_rst(&huart1, &screenData);
	  	} else if (!wifiStat[CWM]) {
			initialise_wifi(&huart1, &screenData, &htim2);
		} else if (!wifiStat[CWJ]) {
			wifi_bread(&huart1, &screenData, &htim2);
		} else if (!wifiStat[USE]) {
			MQTT_use(&huart1, &screenData, &htim2);
		} else if (!wifiStat[CON]) {
			MQTT_connect(&huart1, &screenData);
		} else {
			MQTT_sub(&huart1, &screenData, &htim2);
		}
	  }
	...
	if(touchProcessing == 1)
	{
	  ...
	  touchProcessing = 0;
	  touch_process(&htim2, &screenData);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	// WIFI
		...
		if (mainWifiBuf[uart2Counter] == '\n' && mainWifiBuf[uart2Counter - 1] == '\r') {
			if (return_state(huart1, uart2Counter, &screenData, &htim2)) {
				memset(mainWifiBuf, 0, MAIN_BUF);
				uart2Counter = 0;
			}
		}
...
// Callback function for SPI received from LCD screen
...
// Callback for GPIO from lcd screen
