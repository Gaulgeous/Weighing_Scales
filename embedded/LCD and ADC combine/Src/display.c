...
void weighting_mode(ScreenData *data) {
	...
	if (data->tareState) {
		...
		draw_number(10, 70, Font_7x10, BLACK, data->weight - data->tareValue);
	} else {
...
void counting_mode(ScreenData *data) {
	...
	char buf[12];
	sprintf(buf, "%s %d", "single:",data->single);
...
void calibration(ScreenData *Data) {
	...
	if (Data->cal1Confirm == 1) {
		...
		draw_number(41, 40, Font_7x10, BLACK, Data->calWeight1);
	} else {

		write_number(41, 40, Font_7x10, BLACK, Data->weight);
	}
	...
	if (Data->cal2Confirm == 1) {
		...
		draw_number(41, 66, Font_7x10, BLACK, Data->calWeight2);
	} else {

		write_number(41, 66, Font_7x10, BLACK, Data->weight);
	}
	...
	if (Data->cal3Confirm == 1) {
		...
		draw_number(41, 82, Font_7x10, BLACK, Data->calWeight3);
	} else {

		write_number(41, 92, Font_7x10, BLACK, Data->weight);
	}
...
void touchProcess(TIM_HandleTypeDef *htim2, ScreenData *data) {
	...
	if (data->frame == WEIGHTING) {
		...
		if (data->xValue < 40) {
			...
			if (data->yValue < 56 && data->yValue > 30) {

				data->frame = KEYBOARD;
				data->preMode = CALIBRATION;
				data->cal_num = 1;

			} else if (data->yValue < 82 && data->yValue > 56) {

				data->frame = KEYBOARD;
				data->preMode = CALIBRATION;
				data->cal_num = 2;

			} else if (data->yValue < 110 && data->yValue > 82) {
